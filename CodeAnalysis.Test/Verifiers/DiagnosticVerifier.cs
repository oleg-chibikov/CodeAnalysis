...
namespace Scar.CodeAnalysis.Test.Verifiers
{
    /// <summary>
    ...
    public abstract class DiagnosticVerifier
    {
        ...
        static void VerifyDiagnosticResults(IReadOnlyCollection<Diagnostic> actualResults, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expectedResults)
        {
        ...
        static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, DiagnosticResultLocation expected)
        {
            ...
            Assert.That(
                (actualSpan.Path == expected.Path) || ((actualSpan.Path != null) && actualSpan.Path.Contains("Test0.", StringComparison.OrdinalIgnoreCase) && expected.Path.Contains("Test.", StringComparison.OrdinalIgnoreCase)),
                Is.True,
        ...
        /// <param name="expected">DiagnosticResults that should appear after the analyzer is run on the sources.</param>
        static void VerifyDiagnostics(string[] sources, string language, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expected)
        {
