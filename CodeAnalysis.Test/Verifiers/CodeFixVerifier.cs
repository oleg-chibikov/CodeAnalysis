...
namespace Scar.CodeAnalysis.Test.Verifiers
{
    ...
    public abstract class CodeFixVerifier : DiagnosticVerifier
    {
        ...
        /// introduces other warnings after being applied.
        /// </param>
        static void VerifyFix(string language, DiagnosticAnalyzer analyzer, CodeFixProvider codeFixProvider, string oldSource, string newSource, int? codeFixIndex, bool allowNewCompilerDiagnostics)
        {
            var document = Helpers.DiagnosticVerifier.CreateDocument(oldSource, language);
            var analyzerDiagnostics = Helpers.DiagnosticVerifier.GetSortedDiagnosticsFromDocuments(
                analyzer,
                new[]
                {
                    document
            ...
            for (var i = 0; i < attempts; ++i)
            {
                ...
                analyzerDiagnostics = Helpers.DiagnosticVerifier.GetSortedDiagnosticsFromDocuments(
                    analyzer,
                    new[]
                    {
                        document
